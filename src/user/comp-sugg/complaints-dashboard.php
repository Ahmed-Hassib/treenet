...
// get all complaints
$all_complaints = $comp_sugg_obj->get_all_complaints($_SESSION['sys']['UserID'], $_SESSION['sys']['company_id']);
?>
<div class="container mb-0" dir="<?php echo $page_dir ?>">
  <div class="mb-3">
    <a href="?do=add-comp-sugg&type=0" class="btn btn-outline-primary py-1 fs-12 shadow-sm">
      ...
      <?php echo lang('ADD NEW COMPLAINT', @$_SESSION['sys']['lang']) ?>
    </a>
  </div>
</div>
<!-- start new design -->
    ...
    <div class="col-12">
      <div class="section-block">
        <header class="section-header">
          <h5 class="h5 text-capitalize"><?php echo lang('TOTAL COMPLAINTS', @$_SESSION['sys']['lang']) ?></h5>
          <hr>
        </header>
            ...
            <div class="card card-stat card card-stat shadow-sm border border-1">
              <div class="card-body">
                ...
                <span class="bg-info icon-container <?php echo @$_SESSION['sys']['lang'] == 'ar' ? 'icon-container-left' : 'icon-container-right' ?>">
                  <span class="nums">
                    <?php $all_complaints = $comp_sugg_obj->count_records("`id`", "`comp_sugg`", "WHERE `type` = 0 AND `company_id` = " . $_SESSION['sys']['company_id'] . " AND `added_by` = " . $_SESSION['sys']['UserID']); ?>
